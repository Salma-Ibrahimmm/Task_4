...
describe("AllPerks page (Directory)", () => {
  ...
  */
  test("lists public perks and responds to merchant filtering", async () => {
    ...
    // Render the page again to trigger the fetch
    renderWithRouter(
      <Routes>
        <Route path="/explore" element={<AllPerks />} />
      </Routes>,
    ...
    // Wait for fetch to complete and perks to appear
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });

    // Select merchant filter dropdown and choose the seeded merchant
